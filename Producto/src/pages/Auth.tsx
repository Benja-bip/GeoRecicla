    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      ...
      >
        ...
        <Card className="p-6 shadow-lg overflow-hidden">
          <div className="mb-6">
            {mode === "signup" && step === 2 && (
              <button type="button" onClick={() => setStep(1)} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground transition-colors mb-4">
            ...
            <p className="text-sm text-muted-foreground text-center mt-1">
              {mode === "signin" ? "Bienvenido de vuelta" : step === 1 ? "Paso 1 de 2 — Acceso" : "Paso 2 de 2 — Datos personales"}
            </p>
            {mode === "signup" && (
              <div className="mt-4 h-1.5 bg-secondary rounded-full overflow-hidden">
                <motion.div className="h-full bg-primary rounded-full" initial={{ width: "50%" }} animate={{ width: step === 1 ? "50%" : "100%" }} transition={{ duration: 0.3 }} />
              </div>
            )}
          </div>
          ...
          <AnimatePresence mode="wait">
            {/* SIGNIN */}
            {mode === "signin" && (
              <motion.form key="signin" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} onSubmit={handleSignin} className="space-y-4">
            ...
            {/* SIGNUP STEP 1 */}
            {mode === "signup" && step === 1 && (
              <motion.form key="signup-step1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} onSubmit={handleStep1} className="space-y-4">
                <div className="space-y-2">
            ...
            {/* SIGNUP STEP 2 */}
            {mode === "signup" && step === 2 && (
              <motion.form key="signup-step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} onSubmit={handleSignup} className="space-y-4">
                <div className="space-y-2">
